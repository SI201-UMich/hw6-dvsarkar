...
# e.g.: for debugging and understanding the structure, code, and requests. I used it to help me understand how to parse the JSON responses and how to structure my cache. 
# Asked ChatGPT for help debugging and understanding the JSON structure
#
# Did your use of GenAI on this assignment align with your goals and guidelines in your Gen AI contract? If not, why?
# Yes they did, because I was not copy pasting, instead I was using it to help me understand the structure of the JSON responses rather than take shortcuts.
...
def get_groups_above_cutoff(cutoff, cache_file):
    ...
    for entry in cache.values():
        try:
            ...
            if not group_id:
                continue
            group_counts[group_id] = group_counts.get(group_id, 0) + 1 #AI helped me understand why we need .get to handle missing keys and default to 0
        except (KeyError, TypeError):
            continue
 
    return {gid: count for gid, count in group_counts.items() if count >= cutoff} #AI helpped me with this return statement to filter the groups based on the cutoff
    ...
    for entry in cache.values():
        try:
            ...
            if name.lower() == breed_name.lower():
                found = True
                try:
                    target_group_id = entry['data']['relationships']['group']['data']['id'] #similar to code above
                except (KeyError, TypeError):
                    target_group_id = None
                break
        except (KeyError, TypeError):
